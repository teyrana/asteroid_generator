...
void shuffle_permutation_table(const unsigned int shuffle_count, const unsigned int seed){
    // "p" table -> shuffle() -> "perm" table -> algorithms()

    // copy from initializer array
    for ( int i=0; i<256; i++){
	perm[i]=p[i];
    }
    ...
    int j;
    srand(seed);
    for (int sc=(int)shuffle_count; sc>0; sc--){
	for ( int i=0; i<256; i++){
	    i_value = perm[i];
	    j = (int)((256.0f)*(float)rand()/(float)RAND_MAX);
	    j_value = perm[j];
    ...
    // copy lower table half into upper table half
    for ( int i=0; i<256; i++){
	perm[i+256]=perm[i];
    }
